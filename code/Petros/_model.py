...
class HeteroGCN(torch.nn.Module):
    def __init__(self, iDim, hDim, aggr='pool', oDim=2):
        super(HeteroGCN, self).__init__()
        self.gcn = HeteroGraphConv({
            eType: dgl.nn.pytorch.conv.SAGEConv(iDim, hDim, aggr) for eType in EDGE_TYPES
        }, aggregate='min')
        self.mlpZB = torch.nn.Linear(hDim, hDim)
...
class HeteroMonoGCN(torch.nn.Module):
    ...
    def forward(self, g, useEdgeFeat=False):
        ...
        if useEdgeFeat:
            h = self.gcn(g, g.ndata['nodeFeature'], mod_kwargs={'edge_weight': g.edata['interferCoeff']})
        h = {k: F.leaky_relu(v) for k, v in h.items()}
        h[self.signal] = F.leaky_relu(self.mlpRF(h[self.signal]))

        with g.local_scope():
            g.ndata['h'] = h
            hRF = dgl.max_nodes(g, 'h', ntype=self.signal)
            h = F.relu(self.mlp(hRF))
            return self.regressor(h)
...
class CNNRegressor(torch.nn.Module):
    def __init__(self, hDim, nRF=1, iChannel=1, oDim=2):
        super(CNNRegressor, self).__init__()
        self.conv1 = torch.nn.Conv2d(iChannel, hDim, kernel_size=(3, 1))
        self.conv2 = torch.nn.Conv2d(hDim, hDim, kernel_size=(1, 3))
        self.mlp1 = torch.nn.Linear(hDim, hDim)
        if nRF == 3:
            self.conv1 = torch.nn.Conv2d(iChannel, hDim, kernel_size=(3, 3))
            self.conv2 = torch.nn.Conv2d(hDim, hDim, kernel_size=(1, 3))
            assert(N_TOTAL_AP - 4 > 0)
            self.mlp1 = torch.nn.Linear(hDim*(N_TOTAL_AP-4), hDim)
...
class MLPRegressor(torch.nn.Module):
    def __init__(self, hDim, oDim=2):
        ...
        self.mlp1 = torch.nn.Linear(N_TOTAL_AP*3, hDim)
        self.mlp2 = torch.nn.Linear(hDim, hDim)
        self.regressor = torch.nn.Linear(hDim, oDim)
